...
stories, embeds each sentence with OpenAI `text-embedding-3-small`, and for each
sentence in the beat book finds the most similar source sentence by cosine
similarity.

Ported from the Talbot County beat book pipeline (originally Ollama-based) to
use the OpenAI API that `beat-book-agent` already depends on.
...
# Max inputs per OpenAI embeddings request. The API accepts up to 2048, but we
# cap lower to keep individual HTTP payloads reasonable.
