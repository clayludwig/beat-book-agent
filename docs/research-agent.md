...
- **Geographic gating in the prompt's framing.** The block opens with: *Most are scoped to Chicago, Cook County, or Illinois — match these against the beat's geography and skip any that don't fit.* This is important because the same code path serves non-Chicago beats too (the project also has a `talbot-county-beat-book`); without the gating instruction, an agent working on a Maryland beat would dutifully waste tool calls querying Chicago datasets.
