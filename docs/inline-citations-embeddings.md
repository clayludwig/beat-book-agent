...
- **OpenAI's small embedding model rather than a local one.** OpenAI is already a project dependency for the topic-labeling pipeline upstream; adding a local embedding model would mean shipping weights, GPU detection, and a second model lifecycle. The marginal cost of `text-embedding-3-small` at this scale is too low to argue with.
